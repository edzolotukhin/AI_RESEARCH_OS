...
class QuantitativeUiFacade:
    ...
    def execution_status(self,study_id): return self.service.execution_status(study_id,owner_id=self.owner_id)
    def diagnostics(self,study_id): return self.service.weighting_diagnostics(study_id,owner_id=self.owner_id)
    def run_qc(self,study_id): return self.service.run_default_qc(study_id,owner_id=self.owner_id)
    def approve_qc(self,study_id,**values): return self.service.approve_qc(study_id,owner_id=self.owner_id,actor_id=self.owner_id,**values)
    def construct_weights(self,study_id,targets): return self.service.construct_weights_from_payload(study_id,owner_id=self.owner_id,targets=targets)
    def approve_weights(self,study_id,**values): return self.service.approve_weights(study_id,owner_id=self.owner_id,actor_id=self.owner_id,**values)
    def clean(self,study_id,**values): return self.service.apply_recode_cleaning(study_id,owner_id=self.owner_id,actor_id=self.owner_id,**values)
