...
from application.quantitative.fingerprints import canonical_approval_fingerprint, canonical_digest, canonical_scalar, fingerprint_analysis_specification
...
class QuantitativeAnalysisPlanService:
    ...
    def approve(self,version_id,*,project_id,run_id,new_version_id,approval_id,expected_fingerprint,actor_id,decided_at,rationale,dataset,codebook,weight_sets=()):
        ...
        approval=QuantitativeAnalysisPlanApproval(approval_id,project_id,"QUANTITATIVE",new_version_id,approved.fingerprint,approved.research_design_fingerprint,approved.questionnaire_fingerprint,approved.reconciliation_fingerprint,approved.dataset_fingerprint,approved.codebook_fingerprint,self._weight_fps(approved),coverage.fingerprint,actor_id,decided_at,AnalysisPlanApprovalDecision.APPROVED,rationale,canonical_approval_fingerprint(contract="RC_APPROVAL_V2", project_id=project_id, subject_type="ANALYSIS_PLAN", subject_id=new_version_id, subject_fingerprint=approved.fingerprint, decision="APPROVED", actor_id=actor_id, rationale=rationale, stable_authority={"qz": approved.research_design_fingerprint, "ra": approved.questionnaire_fingerprint, "rb": approved.reconciliation_fingerprint, "dataset": approved.dataset_fingerprint, "codebook": approved.codebook_fingerprint, "weights": self._weight_fps(approved), "coverage": coverage.fingerprint}, digest_provider=self._digest))
    ...
    def reject(self,version_id,*,project_id,run_id,new_version_id,approval_id,expected_fingerprint,actor_id,decided_at,rationale):
        ...
        approval=QuantitativeAnalysisPlanApproval(approval_id,project_id,"QUANTITATIVE",rejected.version_id,rejected.fingerprint,rejected.research_design_fingerprint,rejected.questionnaire_fingerprint,rejected.reconciliation_fingerprint,rejected.dataset_fingerprint,rejected.codebook_fingerprint,self._weight_fps(rejected),rejected.coverage_manifest_fingerprint,actor_id,decided_at,AnalysisPlanApprovalDecision.REJECTED,rationale,canonical_approval_fingerprint(contract="RC_APPROVAL_V2", project_id=project_id, subject_type="ANALYSIS_PLAN", subject_id=rejected.version_id, subject_fingerprint=rejected.fingerprint, decision="REJECTED", actor_id=actor_id, rationale=rationale, stable_authority={"qz": rejected.research_design_fingerprint, "ra": rejected.questionnaire_fingerprint, "rb": rejected.reconciliation_fingerprint, "dataset": rejected.dataset_fingerprint, "codebook": rejected.codebook_fingerprint, "weights": self._weight_fps(rejected), "coverage": rejected.coverage_manifest_fingerprint}, digest_provider=self._digest));self._repository.save_approval(approval,run_id=run_id);return rejected
