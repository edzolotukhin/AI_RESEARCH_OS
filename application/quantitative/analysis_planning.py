...
class QuantitativeAnalysisPlanService:
    ...
    def execution_projection(self,*,quality_assessment=None,**kwargs):
        ...
        value=self.resolve_current_approved(**kwargs); specs=tuple(x.specification for x in value.planned_analyses);comparisons=tuple(x.specification for x in value.planned_comparisons);fp=canonical_digest({"plan":value.fingerprint,"specifications":tuple(x.specification_fingerprint for x in value.planned_analyses),"comparisons":tuple(x.specification_fingerprint for x in value.planned_comparisons),"qc_authority":quality_assessment.fingerprint},digest_provider=self._digest);return AnalysisExecutionProjection(value.version_id,value.fingerprint,quality_assessment.fingerprint,specs,comparisons,fp)
    ...
    def _validate_comparisons(self,items,analyses,design):
        ...
        for item in items:
            ...
            if any(x not in ids for x in item.precursor_analysis_ids) or len(set(item.precursor_analysis_ids))!=2: raise QuantitativeAnalysisPlanError("comparison precursor authority is invalid")
    ...
    def _content(self,plan_id,project_id,d,q,s,r,dataset,codebook,a,c,declarations,assumptions,limitations):
        return {"contract":"RC_CONTENT_V1","plan":plan_id,"project":project_id,"methodology":"QUANTITATIVE","qz":(d.version_id,d.fingerprint),"ra":(q.version_id,q.fingerprint,s.fingerprint),"rb":(r.version_id,r.fingerprint),"dataset":(dataset.version_id,dataset.dataset_fingerprint,dataset.data_fingerprint,dataset.schema_fingerprint),"codebook":(codebook.codebook_version_id,codebook.fingerprint),"analyses":tuple(self._analysis_payload(x) for x in a),"comparisons":tuple((x.planned_comparison_id,x.specification_fingerprint,x.precursor_analysis_ids,x.research_question_ids,x.analytical_requirement_ids,x.expected_result_family,x.assumptions,x.limitations) for x in c),"declarations":tuple((x.requirement_id,x.status.value,x.rationale,x.explicit_multi_component) for x in declarations),"assumptions":assumptions,"limitations":limitations,"version":FINGERPRINT_METHOD_VERSION}
