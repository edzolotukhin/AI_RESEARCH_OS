...
class QuantitativeMeasurementReconciliationService:
    ...
    def approve(self, version_id, *, project_id, run_id, approval_id, expected_fingerprint, actor_id, decided_at, rationale, new_version_id=None, dataset=None, codebook=None):
        ...
        approved_payload={**approved.__dict__, "fingerprint":"", "approval_reference":None, "created_at":"", "created_by":"", "variable_outcomes":tuple(asdict(item) for item in approved.variable_outcomes), "imported_extras":tuple(asdict(item) for item in approved.imported_extras)}
        ...
        approval=QuantitativeMeasurementReconciliationApproval(approval_id,project_id,"QUANTITATIVE",approved.version_id,approved.fingerprint,current.questionnaire_fingerprint,current.expected_measurement_schema_fingerprint,current.dataset_fingerprint,current.data_fingerprint,current.schema_fingerprint,current.codebook_fingerprint,actor_id,decided_at,ReconciliationApprovalDecision.APPROVED,rationale,canonical_approval_fingerprint(contract="RB_APPROVAL_V2", project_id=project_id, subject_type="MEASUREMENT_RECONCILIATION", subject_id=approved.version_id, subject_fingerprint=approved.fingerprint, decision="APPROVED", actor_id=actor_id, rationale=rationale, stable_authority={"questionnaire": current.questionnaire_fingerprint, "expected": current.expected_measurement_schema_fingerprint, "dataset": current.dataset_fingerprint, "data": current.data_fingerprint, "schema": current.schema_fingerprint, "codebook": current.codebook_fingerprint}, digest_provider=self._digest))
