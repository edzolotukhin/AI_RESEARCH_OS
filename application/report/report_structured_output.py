...
class ReportGenerationTelemetry:
...
class ReportStructuredOutputGenerator:
    """Bounded structured-output generation with correction retries for report JSON."""
    ...
    def __init__(
        ...
        *,
        ...
        self._last_telemetry: ReportGenerationTelemetry | None = None

    @property
    def last_telemetry(self) -> ReportGenerationTelemetry | None:
        ...
        *,
        ...
        for attempt in range(1, self._max_attempts + 1):
            ...
            try:
                ...
                self._last_telemetry = ReportGenerationTelemetry(
                    output_tokens=response.output_tokens,
                    reasoning_tokens=response.reasoning_tokens,
                    visible_output_length=response.visible_output_length,
                    finish_reason=response.finish_reason,
        ...
        if last_error is not None:
            raise last_error
        raise StructuredOutputError("Report structured output generation failed.")
...
def _parse_failure_category(error: StructuredOutputError) -> str:
    ...
    if stage in {"extract", "validate", "repair"}:
        return "parse_error"
    return "parse_error"
...
def _build_correction_prompt(
    *,
    ...
    compact_note = (
        "Regenerate compact valid JSON only. Keep sections concise and consolidated. "
        "Do not repeat every finding verbatim."
        if error.is_truncated
        else "Regenerate valid JSON only."
    )
    user = "\n".join(
        [
